...
%%
        ...
        all_ages(i) =    young;
        all_eeg{i} = FullEEG;
%          jesli bym jednak chciala wykluczyc juz w tym miejscu eye chanelle - to sie moze przydac w razie uzycia unfoldu        
...
%         save([path '/mEEG.mat'], 'EEG_model');
        ...
        try
            ...
            data_pro_right{i} = pop_epoch(FullEEG, {'11  '}, [-0.3,1]); %standard data epoching
            ...
            data_pro_right{i} =   pop_rmbase(data_pro_right{i},[-300,0]);  %BASELINE -300MS!
            ...
            data_pro_right{i} = FilterByEventLatency(data_pro_right{i}, 'L_saccade_11', 100, 500); %I'm interested only in saccades with latencies [100;500]ms
            ...
            %IF I WNT TO DIVIDE THEM FOR TWO AGE GROUPS
            OLD_OR_YOUNG = {'old', 'yng'};
...
data_pro_right{1} = MergeSets(data_pro_right_yng{:}) %MERGING SETS - NEEDED FOR AGE COMPARISONS,  s ALL YOUNG PARTICIPANTS AND ALL OLD PARTICIPANTS
...
%%        
for si=1:length(data_anti_right)  %TRICKY PART WITH LOOKING ON 107 ELECTRODE -?> EXPLAINED IN THE CLIP TO BOUNDS FILE 
    ...
    id=d(si).name;
    path = ['/Users/mplome/Desktop/UnfoldEEG/no_unfold/old_yng/F/' num2str(si)];
    ITEMS = {
...
ELECTRODE = [104 9 19];
    I_TITLE = 1;
    I_EEG = 2;
% TIMEPOINTS WERE DETERMINED USING THIS EQUATION ((timepoint*2)- BASELINE = TIME IN MS
    timepoints=[185,200,220];
    ...
    sgtitle([group ' no unfold, age group together, F3, fZ and F4 electrode'])
    for I=1:length(ITEMS)
        ...
        for i=1:3
            ...
            title (num2str((timepoint*2)-300))
        end
        
        %erp
        subplot(4,9,9*(I-1)+[4,5,6])
        %tu dodaje times
        plot(times(1:300), mean(erp(ELECTRODE,1:300),1));
        hold on
        xline(0)
